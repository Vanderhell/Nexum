...
Logical output IDs are unique and limited to 0..255. `NEXUM_PROGRAM_MAX_OUTPUTS` now reflects that representable frozen graph namespace rather than advertising 2048 bindings. Compiler RETRY is single-slot and presently accepts any ACK of the configured type without sequence correlation.
